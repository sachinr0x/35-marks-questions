...
class Hospital:
    def __init__(self, dict_doctor):
        self.dict_doctor= dict_doctor

    def searchByDoctorName(self, input_dName ):
        lst=[]

        for k,v in self.dict_doctor.items():

            if v.doctorName.lower()== input_dName.lower():
                lst.append(v)
    ...
    def calculateConsultationFeeBySpecialization(self, input_specialization):
        total_consultation_fee=0
        for k,v in self.dict_doctor.items():
            if v.specialization== input_specialization:
                total_consultation_fee+= v.consultationFee
        if total_consultation_fee==0:
            return 0
        else:
            return total_consultation_fee
...
for i in range(n):
    ...
    dict_doc_obj[sno]= Doctor(doctorId, doctorName, specialization, consultationFee)
    sno+=1

hospital_obj= Hospital(dict_doc_obj)

input_doctorName=input()
input_specialization=input()

res1= hospital_obj.searchByDoctorName(input_doctorName)
res2= hospital_obj.calculateConsultationFeeBySpecialization(input_specialization)
