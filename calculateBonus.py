...
class Organization():
    ...
    def eligibilityCheck(self, overTimeThreshhold):
      
      for emp in self.employee_list:
          overtime= sum(emp.overtimeContribution.values())
          if overtime>=overTimeThreshhold:
              emp.overtimeStatus= True
          else:
              emp.overtimeStatus= False

    def calculateBonus(self, ratePerHour ):
        total_bonus=0
        for emp in self.employee_list:
            if emp.overtimeStatus:
                total_overtime= sum(emp.overtimeContribution.values())
                total_bonus+= total_overtime* ratePerHour
        return total_bonus
...
for i in range (n):
    ...
    obj_list.append(Employee(eName, designation, salary, overtimeContribution, overtimeStatus= False))
org_obj= Organization(obj_list)

otThreshhold=int(input())
rateph=int(input())

org_obj.eligibilityCheck(otThreshhold)

total_bonus=org_obj.calculateBonus(rateph)

for i in org_obj.employee_list:
